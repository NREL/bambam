use crate::model::output_plugin::mep_output_field::{OPP_FMT_AGGREGATE, OPP_FMT_DISAGGREGATE};
...
pub enum OpportunityCollectFormat {
...
impl OpportunityCollectFormat {
    ...
    pub fn serialize_opportunities(
        &self,
        opportunities: &Vec<(usize, Vec<f64>)>,
        ...
        match self {
            OpportunityCollectFormat::Aggregate => {
                ...
                for (_id, row) in opportunities {
                    for (idx, row_value) in row.iter().enumerate() {
                        acc[idx] += *row_value;
                    }
                }
            ...
            OpportunityCollectFormat::Disaggregate => {
                ...
                for (id, row) in opportunities {
                    ...
                    for (idx, row_value) in row.iter().enumerate() {
                        ...
                        row_obj.insert(activity_type.into(), json!(row_value));
                    }
