...
impl FeatureDependency {
    pub fn as_input_feature(&self) -> (String, InputFeature) {
        match self {
            ...
            FeatureDependency::CustomFeatureCopy { source, unit, .. } => (
                source.clone(),
                InputFeature::Custom {
                    name: source.clone(),
                    unit: unit.to_string(),
    ...
    pub fn apply_feature_dependency(
        ...
        match self {
            FeatureDependency::TimeDependency {
                time_feature,
                destinations,
            } => {
                let (time, time_unit) = state_model.get_time(state, time_feature, None)?;
                for destination in destinations.iter() {
                    state_model.add_time(state, destination, &time, time_unit)?;
                }
                Ok(())
            }
            FeatureDependency::SpeedDependency {
                speed_feature,
                destinations,
            } => {
