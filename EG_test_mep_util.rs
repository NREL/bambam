...
pub enum App { 
    ...
    PreProcessGrid {
        ...
        // grid types: MultiPolygon, GeometryCollection, H3
        grid_type: GridType,
        // file location for output
        output_file: String,
    }
    #[command(
        name = "opps-long", // opps-long
        about = "vectorize an opportunity dataset CSV in long format for bambam integration" // short description via --help
    )]  
    OpportunitiesLongFormat { // mep_util opps-long --vertices_compass_filename FileName.gv --opportunities_filename Filename.csv
    ...
    pub fn run(&self) -> Result<(), String> {
        ...
        */
        match self {
            ...
            use serde_json::json;
            use std::fs;
            use std::io; // for reading lines into output file 
            // unpack the command line arguments into serde_json::Values targeting the Builder and Plugin parameters
            let data = json!({
            ...
            // call GridInputPluginBuilder.build() with the builder arguments Value, producing the plugin
            let instance = GridInputPluginBuilder {};
            let plugin = instance.build(&data);
            
            // call plugin.process() with the plugin arguments Value, executing the plugin
            let processed_plugin = plugin.process(&data);
            ...
            for line in processed_plugin:
            	writeln!(final_file, {}, line);
