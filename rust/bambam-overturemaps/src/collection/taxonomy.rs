...
impl TaxonomyModelBuilder{
    ...
    pub fn build(&self) -> Result<TaxonomyModel, OvertureMapsCollectionError>{
        ...
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .map_err(|e| OvertureMapsCollectionError::TaxonomyLoadingError(format!("Failed to safely create thread to consume taxonomy csv: {e}")))?;
        let taxonomy_tree = runtime.block_on(parse_overturemaps_places_taxonomy_csv(self.source_url.clone()))?;

        // Process records to identify (child, parent) pairs
        let processed_tree_nodes = taxonomy_tree.into_iter().map(|(category, parents)|{
            if parents.len() < 2 { return (category, None) };
...
async fn parse_overturemaps_places_taxonomy_csv(source_url: Option<String>) -> Result<Vec<(String, Vec<String>)>, OvertureMapsCollectionError>{
    ...
    let response = reqwest::get(source_url).await
        .map_err(|e| OvertureMapsCollectionError::TaxonomyLoadingError(format!("GET request failed: {e}")))?;
    let csv_content = response.text().await
        .map_err(|e| OvertureMapsCollectionError::TaxonomyLoadingError(format!("Parsing response failed: {e}")))?;
    ...
    for result in rdr.deserialize() {
        let record: TaxonomyCSVRecord = result.map_err(|e| OvertureMapsCollectionError::TaxonomyDeserializingError(format!("{e}")))?;
...
impl TaxonomyModel {
    pub fn new(tree_nodes: Vec<(String, Option<String>)>, group_mappings: HashMap<String, Vec<String>>) -> Self{
        ...
        let activity_mappings = group_mappings.into_iter()
            .map(|(activity, categories )| (activity, category_tree.get_linearized_query(categories)))
    ...
    pub fn get_unique_categories(&self) -> HashSet<String>{
        ...
        for set in self.group_mappings.values().cloned().collect::<Vec<HashSet<String>>>(){
            result.extend(set);
        }
        result
    }
        ...
        categories.iter().map(|category| {
            group_labels.iter().map(|group|{
                Ok::<bool, OvertureMapsCollectionError>(
                    self.group_mappings.get(group)
                    .ok_or(OvertureMapsCollectionError::GroupMappingError(format!("Group {group} was not found in mapping")))?
                    .contains(category))
...
impl CategoryTree{
    ...
    fn get_linearized_children(&self, node: String) -> Vec<String>{
        ...
        let recursive_children: Vec<String> = node_children
            .iter().flat_map(|e| self.get_linearized_children(e.to_owned()))
            .collect();
    ...
    fn get_linearized_query(&self, query: Vec<String>) -> HashSet<String>{
        ...
        let linearized_children: Vec<String> = query.iter().flat_map(|e| self.get_linearized_children(e.to_owned())).collect();
        
        // Make a HashSet with the linearized query
        HashSet::from_iter(
            query.into_iter()
            .chain(linearized_children.into_iter())
        )
    }
}
